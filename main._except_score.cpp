...
int main(int argc, char** argv) { // 第一个参数是
    ...
    while (fscanf_s(pFile, "%d %d %d %d\n",
        ...
        for (auto i = l.starting_block; i < (l.starting_block + l.size_of_blocks); ++i, ++requestCounter) {
            // 增加计数器
            ...
            TDCParams tdcParams{ i, n ,size };//i对象 n是周期 size缓存大小
    ...
    // 处理完 trace 数据后，打印容器中的记录
    for (const auto& record : trace_records) {
        std::cout << "Starting Block: " << record.starting_block << "\n"
            << "Number of Blocks: " << record.size_of_blocks << "\n"
            << "Ignore: " << record.ignore << "\n"
            << "Request Number: " << record.request_number << "\n"
            << "Current Access Time: " << record.current_time << "\n"
            << "------------------------\n";
    }
